...
//#region ../web-flasher/node_modules/esptool-js/lib/targets/esp32s3.js
var ESP32S3ROM = class extends ESP32ROM {
	constructor() {
		super(...arguments);
	...
	async getChipDescription(loader) {
		...
		}[await this.getPkgVersion(loader)] || "unknown ESP32-S3"} (revision v${majorRev}.${minorRev})`;
	}
	async getPkgVersion(loader) {
		return await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 21 & 7;
	}
	async getRawMinorChipVersion(loader) {
		const hi = await loader.readReg(this.EFUSE_BLOCK1_ADDR + 20) >> 23 & 1;
		const low = await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 18 & 7;
		return (hi << 3) + low;
	}
	...
	async getRawMajorChipVersion(loader) {
		return await loader.readReg(this.EFUSE_BLOCK1_ADDR + 20) >> 24 & 3;
	}
	...
	async getBlkVersionMajor(loader) {
		return await loader.readReg(this.EFUSE_BLOCK2_ADDR + 16) >> 0 & 3;
	}
	async getBlkVersionMinor(loader) {
		return await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 24 & 7;
	}
	...
	async getFlashCap(loader) {
		const addr = this.EFUSE_BASE + 68 + 12;
		return await loader.readReg(addr) >> 27 & 7;
	}
	async getFlashVendor(loader) {
		const addr = this.EFUSE_BASE + 68 + 16;
		return {
			1: "XMC",
			2: "GD",
			3: "FM",
			4: "TT",
			5: "BY"
		}[await loader.readReg(addr) >> 0 & 7] || "";
	}
	async getPsramCap(loader) {
		const addr = this.EFUSE_BASE + 68 + 16;
		return await loader.readReg(addr) >> 3 & 3;
	}
