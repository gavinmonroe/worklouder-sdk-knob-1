...
//#region ../web-flasher/node_modules/esptool-js/lib/targets/esp32s2.js
var ESP32S2ROM = class extends ESP32ROM {
	constructor() {
		super(...arguments);
	...
	async getPkgVersion(loader) {
		const addr = this.EFUSE_BLOCK1_ADDR + 16;
		return await loader.readReg(addr) >> 0 & 15;
	}
	async getMinorChipVersion(loader) {
		const hi = await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 20 & 1;
		const low = await loader.readReg(this.EFUSE_BLOCK1_ADDR + 16) >> 4 & 7;
		return (hi << 3) + low;
	}
	async getMajorChipVersion(loader) {
		return await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 18 & 3;
	}
	async getFlashVersion(loader) {
		return await loader.readReg(this.EFUSE_BLOCK1_ADDR + 12) >> 21 & 15;
	}
	...
	async getPsramVersion(loader) {
		const addr = this.EFUSE_BLOCK1_ADDR + 12;
		return await loader.readReg(addr) >> 28 & 15;
	}
	async getPsramCap(loader) {
		return await this.getPsramVersion(loader);
	}
	async getBlock2Version(loader) {
		const addr = this.EFUSE_BLOCK2_ADDR + 16;
		return await loader.readReg(addr) >> 4 & 7;
	}
	async getChipFeatures(loader) {
		...
		}[await this.getFlashCap(loader)] || "Unknown Embedded Flash";
		features.push(flashDescription);
		const psramDescription = {
			0: "No Embedded Flash",
			1: "Embedded PSRAM 2MB",
			2: "Embedded PSRAM 4MB"
		}[await this.getPsramCap(loader)] || "Unknown Embedded PSRAM";
		features.push(psramDescription);
		const block2VersionDescription = {
			0: "No calibration in BLK2 of efuse",
			1: "ADC and temperature sensor calibration in BLK2 of efuse V1",
			2: "ADC and temperature sensor calibration in BLK2 of efuse V2"
		}[await this.getBlock2Version(loader)] || "Unknown Calibration in BLK2";
	...
	async usingUsbOtg(loader) {
		return (await loader.readReg(this.UARTDEV_BUF_NO) & 255) === this.UARTDEV_BUF_NO_USB_OTG;
	}
