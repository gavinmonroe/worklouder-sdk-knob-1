...
//#region ../web-flasher/node_modules/esptool-js/lib/targets/esp32c2.js
var ESP32C2ROM = class extends ESP32C3ROM {
	...
	async getPkgVersion(loader) {
		const addr = this.EFUSE_BASE + 64 + 4;
		return await loader.readReg(addr) >> 22 & 7;
	}
	async getChipRevision(loader) {
		const block1Addr = this.EFUSE_BASE + 64;
		const pos = 20;
		const addr = block1Addr + 4;
		return (await loader.readReg(addr) & 3 << pos) >> pos;
	}
	...
	async changeBaudRate(loader) {
		if (await this.getCrystalFreq(loader) === 26) loader.changeBaud();
	}
	_d2h(d) {
