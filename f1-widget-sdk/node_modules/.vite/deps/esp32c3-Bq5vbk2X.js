...
//#region ../web-flasher/node_modules/esptool-js/lib/targets/esp32c3.js
var esp32c3_exports = /* @__PURE__ */ __exportAll({ ESP32C3ROM: () => ESP32C3ROM });
var ESP32C3ROM = class extends ESP32ROM {
	constructor() {
		super(...arguments);
	...
	async getPkgVersion(loader) {
		...
		return await loader.readReg(addr) >> 21 & 7;
	}
	async getChipRevision(loader) {
		const block1Addr = this.EFUSE_BASE + 68;
		const pos = 18;
		const addr = block1Addr + 12;
		return (await loader.readReg(addr) & 7 << pos) >> pos;
	}
	async getMinorChipVersion(loader) {
		const hiAddr = this.EFUSE_BASE + 68 + 20;
		const hi = await loader.readReg(hiAddr) >> 23 & 1;
		const lowAddr = this.EFUSE_BASE + 68 + 12;
		const low = await loader.readReg(lowAddr) >> 18 & 7;
		return (hi << 3) + low;
	}
	async getMajorChipVersion(loader) {
		const addr = this.EFUSE_BASE + 68 + 20;
...
//#endregion
export { esp32c3_exports as n, ESP32C3ROM as t };
