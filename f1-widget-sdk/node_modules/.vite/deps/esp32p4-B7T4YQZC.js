import { t as ESP32ROM } from "./esp32-QPtRRPBV.js";
//#region ../web-flasher/node_modules/esptool-js/lib/targets/esp32p4.js
var ESP32P4ROM = class extends ESP32ROM {
	constructor() {
		...
		this.PMU_0P1A_FORCE_TIEH_SEL_0 = 128;
	...
	async getPkgVersion(loader) {
		const addr = this.EFUSE_BLOCK1_ADDR + 8;
		return await loader.readReg(addr) >> 20 & 7;
	}
	async getMinorChipVersion(loader) {
		const addr = this.EFUSE_BLOCK1_ADDR + 8;
		return await loader.readReg(addr) >> 0 & 15;
	}
	...
	async getStubJsonPath(loader) {
		if (await this.getChipRevision(loader) < 300) return "./targets/stub_flasher/stub_flasher_32p4rc1.json";
		else return "./targets/stub_flasher/stub_flasher_32p4.json";
	}
	async getChipDescription(loader) {
		return `${{ 0: "ESP32-P4" }[await this.getPkgVersion(loader)] || "Unknown ESP32-P4"} (revision v${await this.getMajorChipVersion(loader)}.${await this.getMinorChipVersion(loader)})`;
	}
	...
	async getSecureBootEnabled(loader) {
		return (await loader.readReg(this.EFUSE_SECURE_BOOT_EN_REG) & this.EFUSE_SECURE_BOOT_EN_MASK) !== 0;
	}
	/**
	...
	*/
	async getUartdevBufNo(loader) {
		return (await this.getChipRevision(loader) < 300 ? 1341390512 : 1341914800) + 24;
	}
	/**
	...
	*/
	async usesUsbOtg(loader) {
		...
		return (await loader.readReg(uartBufNoAddr) & 255) === this.UARTDEV_BUF_NO_USB_OTG;
	}
	/**
	...
	*/
	async usesUsbJtagSerial(loader) {
		const uartBufNoAddr = await this.getUartdevBufNo(loader);
		return (await loader.readReg(uartBufNoAddr) & 255) === this.UARTDEV_BUF_NO_USB_JTAG_SERIAL;
	}
	async getKeyBlockPurpose(loader, keyBlock) {
		...
		return await loader.readReg(reg) >> shift & 15;
	}
	async isFlashEncryptionKeyValid(loader) {
		...
		if (purposes.some((p) => p === this.PURPOSE_VAL_XTS_AES256_KEY_1) && purposes.some((p) => p === this.PURPOSE_VAL_XTS_AES256_KEY_2)) return true;
		return (await loader.readReg(this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_REG) >> this.EFUSE_FORCE_USE_KEY_MANAGER_KEY_SHIFT & this.FORCE_USE_KEY_MANAGER_VAL_XTS_AES_KEY) !== 0;
	}
	/**
	...
	*/
	async postConnect(loader) {
		if (await this.usesUsbOtg(loader)) loader.ESP_RAM_BLOCK = this.USB_RAM_BLOCK;
		if (!loader.IS_STUB) await this.disableWatchdogs(loader);
	}
	/**
	...
	*/
	checkSpiConnection(loader, spiConnection) {
		if (!spiConnection.every((pin) => pin >= 0 && pin <= 54)) throw new Error("SPI Pin numbers must be in the range 0-54.");
		if (spiConnection.some((pin) => pin === 24 || pin === 25)) loader.debug("GPIO pins 24 and 25 are used by USB-Serial/JTAG, consider using other pins for SPI flash connection.");
	}
	/**
	...
	*/
	async watchdogReset(loader) {
		...
		await loader.writeReg(this.RTC_CNTL_WDTCONFIG0_REG, -805306110);
		await loader.writeReg(this.RTC_CNTL_WDTWPROTECT_REG, 0);
	...
	*/
	async powerOnFlash(loader) {
		if (await this.getChipRevision(loader) <= 300) return;
		...
		await loader.writeReg(this.PMU_DATE_REG, regValue | 3);
