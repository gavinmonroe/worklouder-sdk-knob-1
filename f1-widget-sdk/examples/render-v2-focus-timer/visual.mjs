...
import { VIEWPORT } from "./program.mjs";
...
export function framePng(frame) {
  return encodeRgbaPng(VIEWPORT.width, VIEWPORT.height,
    Buffer.from(rgb565FrameToRgba8888(frame, VIEWPORT)));
}
...
export function contactSheetPng(frames, { scale = 3, gap = 8 } = {}) {
  ...
  for (let index = 0; index < width * height; index += 1) {
    rgba[index * 4] = 9; rgba[index * 4 + 1] = 7; rgba[index * 4 + 2] = 5; rgba[index * 4 + 3] = 255;
  }
