...
if ! debugger_ready; then
  if /usr/bin/pgrep -x input >/dev/null 2>&1; then
    echo "Work Louder Input is already running without the Music bridge."
    echo "Quit Input completely, then run this launcher again."
    exit 1
  fi

  echo "Starting Work Louder Input with its local Music bridge…"
...
echo "Starting Framer F1 Music sync. Keep this window open."
echo "The keyboard must remain connected by USB. Press Control-C to stop."
echo
exec "$NODE_BIN" "$COMPANION_ROOT/runtime/f1-widget-sdk/examples/music-player/companion/run-music-host.mjs"
