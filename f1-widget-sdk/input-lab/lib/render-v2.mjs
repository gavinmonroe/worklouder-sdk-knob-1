...
import { compileInputLabRenderV2Raster, INPUT_LAB_RENDER_V2_RASTER_LIMITS } from "./render-v2-raster.mjs";
...
function normalizeEvent(value, index) {
  ...
  if (value.kind === "input.fn-bottom-knob") {
    invariant(event.flags === 1 && event.id === 1,
      "Fn+bottom-knob simulation requires flags=1 and encoder id=1.");
  }
  if (value.kind === "host.rpc") invariant(Number.isInteger(event.id) && event.id >= 1 && event.id <= 0xffff,
    "Host RPC simulation requires an event id in 1..65535.");
