...
export const WIDGET_SCENE_RPC_METHODS: Readonly<{
...
export const WIDGET_SCENE_RPC_LIMITS: Readonly<{
  maxBundleBytes: number;
  chunkRawBytes: number;
  maxChunks: number;
  maxTransactionIdBytes: number;
  f1wbCommitHeaderBytes: number;
  statusPollMs: number;
  maxStatusPolls: number;
  framebufferBytes: number;
  sceneStoreBytes: number;
  minimumPeakBytes: number;
}>;
export function widgetSceneSha256(value: Uint8Array): string;
export function createWidgetSceneUpload(value: Uint8Array | { binary: Uint8Array }, options: {
  generation: number;
  ...
  bundle: Readonly<{ binary: Buffer; sha256: string; decoded: Record<string, any> }>;
  manifest: Readonly<Record<string, any>>;
  chunks: readonly Readonly<Record<string, any>>[];
  commit: Readonly<Record<string, any>>;
}>;
export function publishWidgetSceneBundle(options: {
  bundle: Uint8Array | { binary: Uint8Array };
  rpc: (method: string, params: Record<string, any>) => Promise<Record<string, any>>;
  expectedProofId: string;
  wait?: (milliseconds: number) => Promise<unknown>;
  maxStatusPolls?: number;
}): Promise<Readonly<{ status: "live-bundle-applied"; mode: "live"; hardwareAccess: true;
  generation: number; sha256: string; bytes: number; chunks: number }>>;
export function publishWidgetSceneBundleSmoke(options: {
  bundle: Uint8Array | { binary: Uint8Array };
  rpc: (method: string, params: Record<string, any>) => Promise<Record<string, any>>;
  confirmed: true;
  expectedGeneration?: number;
  retryBeginOnce?: boolean;
  wait?: (milliseconds: number) => Promise<unknown>;
  beginRetryMs?: number;
  onProgress?: (event: Readonly<{ stage: "uploading-chunks"; current: number; total: number } |
    { stage: "applying-on-keyboard" }>) => void;
}): Promise<Readonly<{ status: "live-canary-commit-acknowledged"; mode: "live-canary";
  hardwareAccess: true; proofBacked: false; uiHandoffVerified: false;
  expectedGeneration: number; generation: number; sha256: string; bytes: number; chunks: number }>>;
