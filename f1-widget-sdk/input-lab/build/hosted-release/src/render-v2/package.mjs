...
const RENDER_V2_CANONICAL_F1WB_BYTES = 62_404;
const RENDER_V2_CANONICAL_F1RA_OFFSET = 332;
const RENDER_V2_CANONICAL_F1RA_BYTES = 62_072;
...
function bytes(value, label) {
  ...
  // Admission helpers own the bytes they validate. Never retain an alias to a
  // caller-controlled ArrayBuffer across digest/structure checks.
...
export function assessRenderV2PackageCompatibility(packageValue,
  ...
  // Compatibility is an admission decision. Always derive it from the actual
  // package bytes, even when a caller supplies an object bearing our format
  // string and plausible-looking cached inspection metadata.
  const inspected = decodeRenderV2Package(packageValue?.binary ?? packageValue);
  const diagnostics = [];
...
function assertCanonicalRenderV2Base(binary, bundleBytes) {
  invariant(bundleBytes === RENDER_V2_CANONICAL_F1WB_BYTES && binary[4] === 1 &&
    binary[5] === 3 && binary[6] === 1 && binary[7] === 0 &&
    binary.readUInt16LE(16) === 104 && binary.readUInt16LE(18) === RENDER_V2_CANONICAL_F1RA_OFFSET,
  "Render v2 package requires the exact 62,404-byte one-slot F1WB base.");
  const descriptor = 20;
  invariant(binary[descriptor] === 1 && binary[descriptor + 1] === 2 &&
    binary[descriptor + 2] >= 1 && binary[descriptor + 2] <= 16 && binary[descriptor + 3] === 0 &&
    binary.readUInt32LE(descriptor + 4) === RENDER_V2_CANONICAL_F1RA_OFFSET &&
    binary.readUInt32LE(descriptor + 8) === RENDER_V2_CANONICAL_F1RA_BYTES &&
    binary.readUInt32LE(descriptor + 12) === 0 && binary.readUInt32LE(descriptor + 16) === 0 &&
    binary.subarray(descriptor + 100, descriptor + 104).every((byte) => byte === 0) &&
    binary.subarray(descriptor + 104, RENDER_V2_CANONICAL_F1RA_OFFSET).every((byte) => byte === 0),
  "Render v2 package F1WB descriptor is not the native canonical one-raster base.");
  const raster = RENDER_V2_CANONICAL_F1RA_OFFSET;
  const record = raster + 64;
  invariant(binary.subarray(raster, raster + 4).toString("ascii") === "F1RA" &&
    binary[raster + 4] === 1 && binary[raster + 5] === 1 &&
    binary.readUInt16LE(raster + 6) === 100 && binary.readUInt16LE(raster + 8) === 310 &&
    binary.readUInt16LE(raster + 10) === 1 &&
    binary.readUInt32LE(raster + 24) === RENDER_V2_CANONICAL_F1RA_BYTES &&
    binary.readUInt32LE(raster + 28) === WIDGET_SCENE_RPC_LIMITS.framebufferBytes &&
    binary[record] === 0 && binary[record + 1] === 0 &&
    binary.readUInt16LE(record + 2) === 0 &&
    binary.readUInt32LE(record + 4) === WIDGET_SCENE_RPC_LIMITS.framebufferBytes,
  "Render v2 package F1RA base does not match native one-frame admission.");
}
...
export function decodeRenderV2Package(value) {
  ...
  invariant(bundleBytes >= RENDER_V2_CANONICAL_F1RA_OFFSET && bundleBytes < binary.length,
    "Render v2 package F1WB prefix length is invalid.");
  assertCanonicalRenderV2Base(binary, bundleBytes);
