 ...
 * focus-clock artifact (15178 bytes, SHA-256 b2eadd58...3705aed),
 * focus-timer artifact (14618 bytes, SHA-256 80e7ca2e...cad8979), and
 * exact blue timer base LZSS (3335 bytes, SHA-256 cae1a090...0427307).
 */
...
/* Exact lzss-1k-len3-66-v1 decoder used by the offline visual compiler.  It
 * writes directly into the borrowed framebuffer and succeeds only when both
 * the complete 62,000-byte output and complete immutable source are consumed. */
static renderer_v2_u32 rv2_decode_lzss(renderer_v2_u8 *dst,
    renderer_v2_u32 dst_bytes, const renderer_v2_u8 *src,
    renderer_v2_u32 src_bytes) {
  renderer_v2_u32 in = 0u, out = 0u;
  if (dst == (renderer_v2_u8 *)0 || src == (const renderer_v2_u8 *)0) return 0u;
  while (out < dst_bytes) {
    renderer_v2_u32 flags, bit;
    ...
    for (bit = 1u; bit <= 0x80u && out < dst_bytes; bit <<= 1u) {
      if ((flags & bit) == 0u) {
        ...
        renderer_v2_u32 code, distance, length, index;
        if (src_bytes - in < 2u) return 0u;
        code = (renderer_v2_u32)src[in] |
          ((renderer_v2_u32)src[in + 1u] << 8u); in += 2u;
        distance = (code & 1023u) + 1u; length = (code >> 10u) + 3u;
        if (distance > out || length > dst_bytes - out) return 0u;
        for (index = 0u; index < length; index++) {
          dst[out] = dst[out - distance]; out++;
        }
      }
    }
  }
...
static renderer_v2_u32 rv2_sha_is_frozen(const renderer_v2_u8 *data, renderer_v2_u32 bytes) {
  ...
  if (bytes == RENDERER_V2_FOCUS_F2EP_BYTES)
    return h[0] == 0xb2eadd58u && h[1] == 0x84c6ee3bu && h[2] == 0x1546ac50u &&
      h[3] == 0xc3c077d1u && h[4] == 0x6eb384adu && h[5] == 0xbf1a8263u &&
      h[6] == 0x1551e69fu && h[7] == 0x93705aedu;
  return bytes == RENDERER_V2_TIMER_F2EP_BYTES &&
    h[0] == 0x80e7ca2eu && h[1] == 0x30a56ca1u && h[2] == 0x2c293202u &&
    h[3] == 0x56884c69u && h[4] == 0xcdaef7feu && h[5] == 0xa27a6468u &&
    h[6] == 0xa0cb5412u && h[7] == 0x2cad8979u;
}

static renderer_v2_u32 rv2_sha_is_timer_base_lzss(const renderer_v2_u8 *data,
    renderer_v2_u32 bytes) {
  renderer_v2_u32 h[8];
  if (data == (const renderer_v2_u8 *)0 || bytes != RENDERER_V2_TIMER_BASE_LZSS_BYTES)
    return 0u;
  rv2_sha_digest(data, bytes, h);
  return h[0] == 0xcae1a090u && h[1] == 0x3e8b9ec0u && h[2] == 0x9880dee0u &&
    h[3] == 0x5652a354u && h[4] == 0xb420cd59u && h[5] == 0x66d10b28u &&
    h[6] == 0xc0382e40u && h[7] == 0xc0427307u;
}
...
static renderer_v2_u32 rv2_sha_is_focus_raster(const renderer_v2_u8 *data,
  ...
  rv2_sha_digest(data, bytes, h);
  return h[0] == 0x4de389c2u && h[1] == 0x25407bc3u &&
    h[2] == 0xd616b0f8u && h[3] == 0x6cfbe2cbu && h[4] == 0x645bda0cu &&
    h[5] == 0xb989c578u && h[6] == 0x5addff67u && h[7] == 0xd72028c7u;
}
...
static renderer_v2_u32 rv2_focus_bundle_is_frozen(const renderer_v2_u8 *bundle,
  ...
  if (d[0] != 1u || d[1] != 2u || d[2] != 10u || d[3] != 0u ||
      ...
      rv2_be32(d + 20u) != 0x4de389c2u || rv2_be32(d + 24u) != 0x25407bc3u ||
      rv2_be32(d + 28u) != 0xd616b0f8u || rv2_be32(d + 32u) != 0x6cfbe2cbu ||
      rv2_be32(d + 36u) != 0x645bda0cu || rv2_be32(d + 40u) != 0xb989c578u ||
      rv2_be32(d + 44u) != 0x5addff67u || rv2_be32(d + 48u) != 0xd72028c7u ||
...
typedef struct {
  ...
  const renderer_v2_u8 *pending_timer_base_lzss;
  renderer_v2_u32 pending_timer_base_lzss_bytes;
  renderer_v2_u32 pending_generation;
  const renderer_v2_u8 *active_focus_bundle;
  const renderer_v2_u8 *active_timer_program;
  const renderer_v2_u8 *active_timer_base_lzss;
  renderer_v2_u32 active_timer_base_lzss_bytes;
...
static renderer_v2_u32 rv2_switch_pending_identity(RendererV2Sidecar *sidecar,
  ...
  sidecar->active_timer_base_lzss = sidecar->pending_timer_base_lzss;
  sidecar->active_timer_base_lzss_bytes = sidecar->pending_timer_base_lzss_bytes;
...
static renderer_v2_u32 rv2_timer_base_refresh_identity(RendererV2Sidecar *sidecar,
  ...
  admitted = error == 0u && sidecar->active_profile == RV2_PROFILE_FOCUS &&
    ...
    sidecar->active_timer_base_lzss != (const renderer_v2_u8 *)0 &&
...
static void renderer_v2_timer_tick(RendererV2TimerProxy *proxy) {
  ...
  if (!rv2_decode_lzss(backend + 160u, RENDERER_V2_FRAME_BYTES,
      sidecar->active_timer_base_lzss,
      sidecar->active_timer_base_lzss_bytes)) {
    __atomic_store_n(&sidecar->timer_admitted, 0u, __ATOMIC_RELEASE);
    return;
  }
...
renderer_v2_u32 renderer_v2_native_prepare(void *controller,
  ...
  const renderer_v2_u8 *bundle, *program, *timer_program, *timer_base_lzss;
  ...
  timer_base_lzss = timer_program == (const renderer_v2_u8 *)0 ? timer_program :
    timer_program + RENDERER_V2_TIMER_F2EP_BYTES;
  if (package == (const renderer_v2_u8 *)0 ||
      ...
      !rv2_sha_is_timer_base_lzss(timer_base_lzss,
        RENDERER_V2_TIMER_BASE_LZSS_BYTES) ||
  ...
  sidecar->pending_timer_base_lzss = timer_base_lzss;
  sidecar->pending_timer_base_lzss_bytes = RENDERER_V2_TIMER_BASE_LZSS_BYTES;
...
renderer_v2_u32 renderer_v2_native_cancel(void *controller) {
  ...
  sidecar->pending_timer_base_lzss = (const renderer_v2_u8 *)0;
  sidecar->pending_timer_base_lzss_bytes = 0u;
...
renderer_v2_u32 renderer_v2_host_decode_timer_base(renderer_v2_u8 *frame,
  ...
  return frame_bytes == RENDERER_V2_FRAME_BYTES &&
    rv2_sha_is_timer_base_lzss(compressed, compressed_bytes) &&
    rv2_decode_lzss(frame, frame_bytes, compressed, compressed_bytes);
}
