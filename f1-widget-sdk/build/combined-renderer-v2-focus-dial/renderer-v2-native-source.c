 ...
 * focus-dial artifact (15162 bytes, SHA-256 37397f94...eaef4255).
 */
...
static renderer_v2_u32 rv2_sha_is_frozen(const renderer_v2_u8 *data, renderer_v2_u32 bytes) {
  ...
  return bytes == RENDERER_V2_FOCUS_F2EP_BYTES &&
    h[0] == 0x37397f94u && h[1] == 0xe557bea3u && h[2] == 0x99b748beu &&
    h[3] == 0x788914bdu && h[4] == 0xa64a169au && h[5] == 0xbc98207du &&
    h[6] == 0x30b110e6u && h[7] == 0xeaef4255u;
}
...
static renderer_v2_u32 rv2_sha_is_focus_raster(const renderer_v2_u8 *data,
  ...
  rv2_sha_digest(data, bytes, h);
  return h[0] == 0xf25e300fu && h[1] == 0x81d2e6e8u &&
    h[2] == 0x8437d8f3u && h[3] == 0x3433fb0du && h[4] == 0x2c6141c6u &&
    h[5] == 0x0d61e8aeu && h[6] == 0xd1efd1ddu && h[7] == 0x0ddc5edfu;
}
...
static renderer_v2_u32 rv2_focus_bundle_is_frozen(const renderer_v2_u8 *bundle,
  ...
  if (d[0] != 1u || d[1] != 2u || d[2] != 10u || d[3] != 0u ||
      ...
      rv2_be32(d + 20u) != 0xf25e300fu || rv2_be32(d + 24u) != 0x81d2e6e8u ||
      rv2_be32(d + 28u) != 0x8437d8f3u || rv2_be32(d + 32u) != 0x3433fb0du ||
      rv2_be32(d + 36u) != 0x2c6141c6u || rv2_be32(d + 40u) != 0x0d61e8aeu ||
      rv2_be32(d + 44u) != 0xd1efd1ddu || rv2_be32(d + 48u) != 0x0ddc5edfu ||
...
renderer_v2_u32 renderer_v2_native_prepare(void *controller,
  ...
  if (bundle == (const renderer_v2_u8 *)0 || program == (const renderer_v2_u8 *)0 ||
      bundle_bytes != RENDERER_V2_FOCUS_F1WB_BYTES ||
      program_bytes != RENDERER_V2_FOCUS_F2EP_BYTES ||
      !rv2_is_data(bundle, bundle_bytes) || !rv2_is_data(program, program_bytes) ||
      program != bundle + RENDERER_V2_FOCUS_F1WB_BYTES ||
      !rv2_focus_bundle_is_frozen(bundle, bundle_bytes, generation) ||
      !rv2_sha_is_frozen(program, program_bytes) ||
      !renderer_v2_runtime_init(&sidecar->pending_runtime, program, program_bytes)) {
  ...
  sidecar->pending_bundle = bundle; sidecar->pending_bundle_bytes = bundle_bytes;
  sidecar->pending_program = program; sidecar->pending_program_bytes = program_bytes;
