 ...
 */
static void renderer_scene_rpc_generic_invalidate_hook(void *state);
#define SCENE_INVALIDATE_HOOK(state) \
  renderer_scene_rpc_generic_invalidate_hook((void *)(state))
...
static void renderer_scene_rpc_generic_invalidate_hook(void *opaque) {
  generic_abort_upload((RendererSceneRpcState *)opaque);
}
...
s32 renderer_scene_rpc_core_commit(RendererSceneRpcState *state,
  ...
  if (transaction_id_length != state->transaction_id_length ||
    ...
    invalidate(state); return SCENE_RPC_PARAMS;
  }
  ...
  if (!equal_bytes(actual_sha, expected_sha, 32u)) {
    invalidate(state); return SCENE_RPC_SHA;
  }
#ifdef RENDERER_SCENE_RPC_HOST_TEST
