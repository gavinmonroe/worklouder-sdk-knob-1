...
const source = path.join(projectRoot, "custom-firmware/experimental/stage3e-wpm-sprite.S");
const linkerScript = path.join(projectRoot, "custom-firmware/experimental/stage3e-wpm-sprite.ld");
const pinnedHexPath = path.join(projectRoot, "custom-firmware/experimental/stage3e-wpm-sprite.hex");
...
const expected = Object.freeze({
  ...
  textSize: 0x43c,
  binarySize: 0x4f8,
  sha256: "e96498a5a7dde80dff9bd043554463a5b48b28ebc5d87091bc625afb52f405f3",
  symbols: Object.freeze({
    stage3e_screen_setup_wrapper: Object.freeze({ address: 0x42116fcc, size: 0xa2 }),
    stage3e_make_image: Object.freeze({ address: 0x42117070, size: 0x30 }),
    stage3e_make_label: Object.freeze({ address: 0x421170a0, size: 0x38 }),
    stage3e_wpm_build: Object.freeze({ address: 0x421170d8, size: 0x9d }),
    stage3e_wpm_cleanup: Object.freeze({ address: 0x42117178, size: 0x13 }),
    stage3e_wpm_id: Object.freeze({ address: 0x4211718c, size: 0x07 }),
    stage3e_sample_and_render: Object.freeze({ address: 0x42117194, size: 0x228 }),
    stage3e_wpm_ui_refresh: Object.freeze({ address: 0x421173bc, size: 0x4c }),
  }),
  ...
  forbiddenLiterals: Object.freeze([
    0x4206eae0, // No global key hook in Stage-3E code.
...
function functionBody(disassembly, symbol) {
  ...
  assertCondition(start >= 0, `Stage-3E symbol ${symbol} is absent from disassembly.`);
  const next = disassembly.indexOf("\n\n", start);
  return disassembly.slice(start, next < 0 ? undefined : next);
}

const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "framer-stage3e-abi-"));
...
try {
  ...
  assertCondition(/file format elf32-xtensa-le/u.test(header.stdout),
    "Stage-3E did not link as ESP32-S3 little-endian ELF.");
  assertCondition(/\.stage3e_literal\s+000000bc\s+42116f10\s+42116f10/u.test(header.stdout),
    "Stage-3E literal address or size changed.");
  assertCondition(/\.stage3e_text\s+0000043c\s+42116fcc\s+42116fcc/u.test(header.stdout),
    "Stage-3E text address or size changed.");
  ...
  assertCondition(/There are no relocations in this file\./u.test(relocations.stdout),
    "Stage-3E contains a final-link relocation.");
  ...
  for (const [name, wanted] of Object.entries(expected.symbols)) {
    ...
    assertCondition(actual !== undefined, `Stage-3E symbol ${name} is missing.`);
    assertCondition(actual.address === wanted.address && actual.size === wanted.size,
      `Stage-3E symbol ${name} moved or changed size.`);
  }

  const disassembly = (await run(tool("objdump"), ["-d", elfPath], {
    maxBuffer: 1024 * 1024,
  })).stdout;
  const build = functionBody(disassembly, "stage3e_wpm_build");
  const sample = functionBody(disassembly, "stage3e_sample_and_render");
  const refresh = functionBody(disassembly, "stage3e_wpm_ui_refresh");
  ...
  "Stage-3E no longer creates sky, cat, WPM, and stats objects in pinned z-order.");
  assertCondition(/movi(?:\.n)?\s+a12, -36/u.test(build) &&
    /movi(?:\.n)?\s+a12, 42/u.test(build),
  "Stage-3E WPM/stats label positions changed.");
  ...
  assertCondition(/^[0-9a-f]+$/u.test(pinnedHex) && pinnedHex.length % 2 === 0,
    "Pinned Stage-3E ABI is not canonical lowercase hex.");
  assertCondition(binary.equals(Buffer.from(pinnedHex, "hex")),
    "Assembled Stage-3E differs from the pinned artifact.");
  assertCondition(binary.length === expected.binarySize,
    `Stage-3E binary size changed to ${binary.length}.`);
  const digest = createHash("sha256").update(binary).digest("hex");
  assertCondition(digest === expected.sha256, `Stage-3E binary hash changed to ${digest}.`);
  for (const value of expected.requiredLiterals) {
    assertCondition(countU32(binary, value) >= 1,
      `Stage-3E required literal 0x${value.toString(16)} is absent.`);
  }
  for (const value of expected.forbiddenLiterals) {
    assertCondition(countU32(binary, value) === 0,
      `Stage-3E references forbidden value 0x${value.toString(16)}.`);
  }
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    renderCaches: [100, 104, 108, 112, 116],
  }, null, 2)}\n`);
} finally {
