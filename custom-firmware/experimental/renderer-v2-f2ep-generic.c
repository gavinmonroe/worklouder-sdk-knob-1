...
static renderer_v2_u32 rv2_owned_active_identity(
  ...
  return sidecar->owned_valid != 0u &&
    *(const renderer_v2_u8 **)(controller + 28u) == sidecar->owned_bundle &&
    *(renderer_v2_u32 *)(controller + 32u) == sidecar->owned_bundle_bytes &&
    *(renderer_v2_u32 *)(controller + 44u) == sidecar->owned_generation &&
    *(renderer_v2_u32 *)(controller + 64u) == 0u;
}
...
static void renderer_v2_generic_live_tick(void *controller) {
  ...
  if (__atomic_load_n(&sidecar->detach_requested, __ATOMIC_ACQUIRE) != 0u) {
    ...
    if (*(const renderer_v2_u8 **)(bytes + 28u) == (const renderer_v2_u8 *)0 &&
        *(const renderer_v2_u8 **)(bytes + 62160u) == (const renderer_v2_u8 *)0)
  ...
  if (__atomic_load_n(&sidecar->switch_state, __ATOMIC_ACQUIRE) ==
      ...
      *(const renderer_v2_u8 **)(bytes + 28u) == sidecar->owned_bundle &&
      *(renderer_v2_u32 *)(bytes + 32u) == sidecar->pending_bundle_bytes &&
      *(renderer_v2_u32 *)(bytes + 44u) == sidecar->pending_generation &&
...
RV2_EXPORT void *renderer_v2_native_attach(void *setup_owner, void *registry,
...
#else
  {
    ...
    rv2_barrier(); *(void ***)controller = sidecar->vtable; rv2_barrier();
    return sidecar;
  }
#endif
}
...
RV2_EXPORT renderer_v2_u32 renderer_v2_native_begin_upload(void *controller) {
  ...
  if (sidecar->owned_valid != 0u && sidecar->owned_generation != 0u &&
      (__atomic_load_n(&sidecar->activation_observed, __ATOMIC_ACQUIRE) == 0u ||
       !rv2_owned_active_identity(sidecar, bytes) ||
       (sidecar->owned_is_v2 != 0u &&
  ...
  if (sidecar->owned_valid != 0u &&
      *(const renderer_v2_u8 **)((renderer_v2_u8 *)controller + 28u) ==
        sidecar->owned_bundle) {
