...
struct __attribute__((aligned(16))) physical_block {
    ...
    __attribute__((aligned(16))) uint8_t vm_heap[FRAMER_F2JS_HEAP_BYTES];
};
...
_Static_assert((offsetof(physical_block, static_task) & 15u) == 0u &&
               (offsetof(physical_block, vm_heap) & 15u) == 0u,
...
static void *platform_allocate(void *opaque, size_t bytes)
{
    ...
    if (block == (physical_block *)0 || bytes != sizeof(block->vm_heap) ||
        block->heap_claimed != 0u)
        return (void *)0;
    block->heap_claimed = 1u;
    zero_bytes(block->vm_heap, sizeof(block->vm_heap));
...
{
    physical_block *block = (physical_block *)opaque;
    if (block != (physical_block *)0 && allocation == block->vm_heap) {
        zero_bytes(block->vm_heap, sizeof(block->vm_heap));
        block->heap_claimed = 0u;
    }
}
...
{
    return (id >= 0xb240u && id <= 0xb244u) || id == 0xb24du ||
