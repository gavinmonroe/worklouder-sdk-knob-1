...
const includes = [build, diagSource, canary, vendor, resident, target, runtimeProof, path.join(here, "psram-module-src"), here];
...
const sources = [
  ...
  [path.join(here, "psram-module-src/physical_integration.c"), "physical.o",
    ["-DFRAMER_RUNTIME_PROOF_EXACT_ABI_ACK=0x36317013u"]],
];
const moduleObjects = [];
...
const textFile = path.join(output, "mqjs-id28-text-page-psram.bin");
const rodataFile = path.join(output, "mqjs-id28-rodata-page-psram.bin");
await Promise.all([
...
const slotFile = path.join(output, "mqjs-id28-slot-a-psram.bin");
...
invariant(Math.abs((expected.releaseBlockBytes - blockBytes) - heapBytes) <= 16,
  `PSRAM block size ${blockBytes} did not shrink the frozen ` +
  `${expected.releaseBlockBytes} B block by the ${heapBytes} B VM heap ` +
  `(one 16-byte alignment quantum of slack): the heap did not leave internal RAM.`);
...
const loaderFile = path.join(output, "mqjs-id28-resident-loader-psram.bin");
await run(xtensa("objcopy"), ["-O", "binary", "-j", ".text", loaderElf, loaderFile]);
const loaderRaw = await readFile(loaderFile);
await writeFile(path.join(output, "resident-loader-psram.dis.txt"), disassembly.stdout);
...
const appFile = path.join(output, "framer-0.4.1-mqjs-id28-PSRAM-module-app.bin");
await writeFile(appFile, repaired);
await copyFile(assetPaths.source, path.join(output, "weather-id28-gen19.js"));

const manifest = {
  format: "framer-f1-mquickjs-psram-heap-module-build-v1",
  purpose: "Move the 64 KiB MicroQuickJS heap out of the resident internal-RAM block into PSRAM (MALLOC_CAP_SPIRAM|MALLOC_CAP_8BIT, validated inside 0x3c1d0000..0x3c3d0000), keeping the diag4 last_error instrumentation",
...
await writeFile(path.join(output, "psram-module-manifest.json"),
  `${JSON.stringify(manifest, null, 2)}\n`);
process.stdout.write(`${JSON.stringify({
  status: "PASS_PSRAM_MODULE_BUILT_NO_HARDWARE",
