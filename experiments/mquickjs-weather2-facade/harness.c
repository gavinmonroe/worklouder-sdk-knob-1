/* Freestanding-C oracle check for the restyled gen19 facade asset.
 *
 * Same shape as experiments/mquickjs-target-facade/host_harness.c, but the
 * expected generation is an argument instead of a literal 18, so the exact
 * on-device target_facade.c can be run against the new asset without editing
 * any release source. */
...
static void hex32(const char *text, uint8_t output[32])
{
    ...
    for (i = 0u; i < 32u; ++i) {
        unsigned int value; assert(sscanf(text + i * 2u, "%2x", &value) == 1);
...
int main(int argc, char **argv)
{
    ...
    uint32_t generation, count, index;
    uint16_t frame[FRAMER_TF_CANVAS_PIXELS];
    FILE *output;
    /* asset base cases out-frames f2js-sha contract-sha generation */
    assert(argc == 8);
    ...
    generation = (uint32_t)strtoul(argv[7], NULL, 10);
    ...
    assert(asset_bytes <= FRAMER_TF_MAX_ASSET_BYTES);
    admitted = framer_tf_admit(&context, asset, asset_bytes, (const uint16_t *)base_raw,
                               FRAMER_TF_CANVAS_PIXELS, generation, f2js, contract,
                               0x12345678u);
    assert(admitted == FRAMER_TF_OK);
    output = fopen(argv[4], "wb"); assert(output != NULL);
    printf("{\"status\":\"PASS_HOST_C_WEATHER2_FACADE\",\"assetBytes\":%u,\"results\":[",
           (unsigned int)asset_bytes);
    for (index = 0u; index < count; ++index) {
        ...
        result = framer_tf_render(&context, &mailbox, frame, FRAMER_TF_CANVAS_PIXELS,
                                  0x12345678u, &metrics);
        assert(fwrite(frame, sizeof(uint16_t), FRAMER_TF_CANVAS_PIXELS, output) ==
               FRAMER_TF_CANVAS_PIXELS);
        printf("%s{\"result\":%u,\"writes\":%u,\"targets\":%u,\"revision\":%u}",
               index ? "," : "", (unsigned int)result, metrics.overlay_writes,
               metrics.formatted_targets, metrics.applied_revision);
    }
    assert(fclose(output) == 0);
    {   /* A torn mailbox must retire to the pristine admitted base. */
        framer_tf_mailbox mailbox; framer_tf_metrics metrics; framer_tf_result result;
    ...
    {   /* Bit-rot, an out-of-range slot, escaped geometry, a foreign generation
         * and an exhausted overlay budget must each fail closed. */
        ...
        assert(framer_tf_admit(&rejected, mutated, asset_bytes, (const uint16_t *)base_raw,
                               FRAMER_TF_CANVAS_PIXELS, generation, f2js, contract, 1u) ==
               FRAMER_TF_ERR_CRC);
        memcpy(mutated, asset, asset_bytes);
        mutated[FRAMER_TF_HEADER_BYTES + 3u * 40u + 26u] = 16u; reseal(mutated, asset_bytes);
        assert(framer_tf_admit(&rejected, mutated, asset_bytes, (const uint16_t *)base_raw,
                               FRAMER_TF_CANVAS_PIXELS, generation, f2js, contract, 1u) ==
               FRAMER_TF_ERR_MALFORMED);
        ...
        memcpy(mutated, asset, asset_bytes);
        assert(framer_tf_admit(&rejected, mutated, asset_bytes, (const uint16_t *)base_raw,
                               FRAMER_TF_CANVAS_PIXELS, generation + 1u, f2js, contract, 1u) ==
               FRAMER_TF_ERR_MALFORMED);
        memcpy(mutated, asset, asset_bytes); write_u32(mutated + 32u, 1u);
    ...
    printf("],\"torn\":\"PASS\",\"malformed\":\"PASS\",\"generation\":\"PASS\","
           "\"overflow\":\"PASS\"}\n");
